...
/**
 * Gets the restrictions for a course
 * @title Get restrictions for course.
...
const myAction = async () => {
  ...
  try {
    ...
    const text = 'SELECT restriction FROM COURSES WHERE CODE LIKE $1 LIMIT 1'
    ...
    await client
      ...
      .then(res => {
        if (Object.keys(res.rows).length === 0) {
          ...
          if (res.rows[0].restriction == '') {
            fullString = fullString + 'There are no restrictions.'
          } else {
            fullString = fullString + res.rows[0].restriction.replace('Restriction: ', '') + '\n'
          }
        }
